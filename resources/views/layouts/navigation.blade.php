    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                {{-- Navegación principal (escritorio), oculta en dashboard de selección --}}
                @unless(request()->routeIs('dashboard'))
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        {{-- Inicio --}}
                        <x-nav-link
                          :href="route($dashboardRoute)"
                          :active="request()->routeIs(['admin.dashboard','medico.dashboard','paciente.dashboard'])"
                        >
                            {{ __('Inicio') }}
                        </x-nav-link>
                        ...
                        @if(Auth::user()->hasRole($Rol::ADMINISTRADOR) && request()->is('admin*'))
                            ...
                            <x-nav-link :href="route('admin.bloqueos.index')"  :active="request()->routeIs('admin.bloqueos.*')">  {{ __('Bloqueos de Agenda') }} </x-nav-link>
                            <x-nav-link :href="route('admin.pacientes.index')" :active="request()->routeIs('admin.pacientes.*')"> {{ __('Pacientes') }}           </x-nav-link>
                        @endif
                    ...
                    <x-slot name="trigger">
                        ...
                        >
                            <div>
                                ...
                                {{-- Lista de todos los roles asignados --}}
                                @php
                                    $allRoles = Auth::user()->roles->pluck('rol')->implode(', ');
                                @endphp
                                ({{ $allRoles }})
                            </div>
                            <div class="ms-1">
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @unless(request()->routeIs('dashboard'))
                @if(Auth::user()->hasRole($Rol::ADMINISTRADOR) && request()->is('admin*'))
                    ...
                    <x-responsive-nav-link :href="route('admin.bloqueos.index')"  :active="request()->routeIs('admin.bloqueos.*')">  {{ __('Bloqueos de Agenda') }} </x-responsive-nav-link>
